...
module.exports = HandleMsg = async (aruga, message) => {
        ...
        function isStickerMsg(id){
            ...
            for (let i of stickerspam){
                if(i.id === id){
                    if (i.msg >= 7) {
                        found === true 
                        aruga.reply(from, '*[ANTI STICKER SPAM]*\nSi haces spam con stickers te pateare em trasero ', message.id).then(() => {
        ...
        if (isGroupMsg && GroupLinkDetector && !isGroupAdmins && !isOwner){
            if (chats.match(/(https:\/\/chat.whatsapp.com)/gi)) {
                ...
                if (!check) {
                    ...
                    aruga.reply(from, '*[GROUP LINK DETECTOR]*\nSi envías un enlace te pateare el trasero fuera de este grupo :(', id).then(() => {
	...
	// Filter Banned People
        ...
        case 'help':
            ...
            .then(() => ((isGroupMsg) && (isGroupAdmins)) ? aruga.sendText(from, `Menu Admin Grupo: *${prefix}menuadmin*`) : null)
            break
        case 'menuadmin':
            if (!isGroupMsg) return aruga.reply(from, 'Lo sentimos, este comando sólo se puede utilizar en grupos!', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Este comando solo puede ser usado por los admins facheros 😎!', id)
        ...
        case 'ownerbot':
            ...
            .then(() => aruga.sendText(from, 'Si desea solicitar la función, por favor chatee el número de propietario!'))
            break
        case 'join':
            if (args.length == 0) return aruga.reply(from, `Si desea unir el bot a un grupo \nUse ${prefix}join [link group]No lo haga sin autorización`, id)
            let linkgrup = body.slice(6)
            let islink = linkgrup.match(/(https:\/\/chat.whatsapp.com)/gi)
            let chekgrup = await aruga.inviteInfo(linkgrup)
            if (!islink) return aruga.reply(from, 'Error en el enlace', id)
            if (isOwnerBot) {
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          await aruga.sendText(from, 'Se a unido con éxito al grupo!')
                          await aruga.sendText(chekgrup.id, `Hola soy samurái Bot puedes usar  ${prefix}samurai para ver mi lista de comandos`)
                      })
            } else {
                let cgrup = await aruga.getAllGroups()
                if (cgrup.length > groupLimit) return aruga.reply(from, `Lo siento grupo lleno \nMaximo del  Groupo es: ${groupLimit}`, id)
                if (cgrup.size < memberLimit) return aruga.reply(from, `Lo sentimos, Bot no se unirá si los miembros del grupo no exceden${memberLimit} personas`, id)
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () =>{
                          await aruga.reply(from, 'Unirse con éxito al grupo a través de enlace!', id)
                      })
                      .catch(() => {
                          aruga.reply(from, 'Fallo!😖', id)
                      })
            }
            break
        case 'botstat': {
	...
	case 'stimg':
            if (quotedMsg && quotedMsg.type == 'sticker') {
                ...
                aruga.reply(from, `Awanta wee no soy flahs espera un momento...`, id)
                const imageBase64 = `data:${quotedMsg.mimetype};base64,${mediaData.toString('base64')}`
                await aruga.sendFile(from, imageBase64, 'imgsticker.jpg', 'Berhasil convert Sticker to Image!', id)
                .then(() => {
                    console.log(`Wee la pegatina solo puede durar ${processTime(t, moment())} Segundos`)
                })
        } else if (!quotedMsg) return aruga.reply(from, `Error😖Lo siento wee!`, id)
        break
			
			
        // Sticker Creator
    case 'logopornhub':
            if (args.length === 1) return aruga.reply(from, `Usar el comando *${prefix}logopornhub [ |Teks1|Teks2 ]*,\n\n Ejemplo : *${prefix}logopornhub |Samurai| Bot*`, id)
            argz = body.trim().split('|')
            if (argz.length >= 2) {
                aruga.reply(from, `sabar brok eug proses dolo....`, id)
                const lpornhub = argz[1]
                const lpornhub2 = argz[2]   
                if (lpornhub > 10) return aruga.reply(from, '*Texto 1 demaciado largo!*\n_Limite de 10 letras!_', id)
                if (lpornhub2 > 10) return aruga.reply(from, '*Texto 2 demaciado largo!*\n_Limite de 10 letras!_', id)
                aruga.sendFileFromUrl(from, `https://docs-jojo.herokuapp.com/api/phblogo?text1=${lpornhub}&text2=${lpornhub2}`)
            } else {
                await aruga.reply(from, `Formato incorrecto Wee!\n[❗] Use el comando *${prefix}logopornhub [ |Texto1| Texto2 ]*,\n\n Ejemplo : *${prefix}logopornhub |Samurai| Bot*`, id)
            }
            break
	case 'coolteks':
	case 'cooltext':
            if (args.length == 0) return aruga.reply(from, `Para crear cool Text CoolText en la imagen, utilice ${prefix}cooltext teks\n\nEjemplo: ${prefix}Samurai Bot`, id)
        ...
        case 's':
            if ((isMedia || isQuotedImage) && args.length === 0) {
                ...
                .then(() => {
                    aruga.reply(from, 'Qui\'s tu Sticker')
                    console.log(`Solo debe tardar ${processTime(t, moment())} Segundos`)
                })
            } else if (args[0] === 'nobg') {
                if (isMedia || isQuotedImage) {
                    ...
                    console.log(err)
	   	            await aruga.reply(from, 'Lo siento limite alcanzado :v', id)
                    }
                }
            } else if (args.length === 1) {
                if (!isUrl(url)) { await aruga.reply(from, 'Wee enlace invalido.', id) }
                aruga.sendStickerfromUrl(from, url).then((r) => (!r && r !== undefined)
                    ? aruga.sendText(from, 'Wee el enlace no tenia la imagen.')
                    : aruga.reply(from, 'Aqui\'s Tu Sticker')).then(() => console.log(`Solo debe tardar  ${processTime(t, moment())} Segundos`))
            } else {
                await aruga.reply(from, `Sin fotos :(! Para usar ${prefix}sticker\n\n\nDerechos de autor\n${prefix}sticker <biasa>\n${prefix}sticker nobg <Con fondo>\n\natau Use \n${prefix}sticker <link img>`, id)
            }
            break
            case 'antisticker':
            case 'antistiker':
                    if (!isGroupMsg) return aruga.reply(from, 'Lo sentimos, este comando sólo se puede utilizar en grupos!', id)
                    if (!isGroupAdmins) return aruga.reply(from, 'Este comando solo puede ser usado por los admin facheros 😎!', id)
                    if (!isBotGroupAdmins) return aruga.reply(from, 'Necesito admin para esta función:)', id)
                    if (args[0] == 'on') {
                        var cek = antisticker.includes(chatId);
                        if(cek){
                            return aruga.reply(from, '*Anti spam de stickers activado*', id) //if number already exists on database
                        } else {
                            antisticker.push(chatId)
                            fs.writeFileSync('./lib/helper/antisticker.json', JSON.stringify(antisticker))
                            aruga.reply(from, '*[Anti Sticker SPAM]* ATENCIÓN\Si envías mas de 7 Stickers te pateare afuera del grupo:v!', id)
                        }
                    } else if (args[0] == 'off') {
                        var cek = antilink.includes(chatId);
                        if(cek){
                            return aruga.reply(from, '*Anti spam de stickers desactivado con éxito', id) //if number already exists on database
                        } else {
                            let nixx = antisticker.indexOf(chatId)
                            antisticker.splice(nixx, 1)
                            fs.writeFileSync('./lib/helper/antisticker.json', JSON.stringify(antisticker))
                            aruga.reply(from, '*[Anti Sticker SPAM]* Se a desactivado\n', id)
                        }
                    } else {
                    ...
                    if (args[0] == 'on') {
                        var cek = antilink.includes(chatId);
                        if(cek){
                            return aruga.reply(from, '*Antilink ha sido activado en el grupo', id) //if number already exists on database
                        } else {
                            antilink.push(chatId)
                            fs.writeFileSync('./lib/helper/antilink.json', JSON.stringify(antilink))
                            aruga.reply(from, '*[ANTILINK]*Se ha activado\nSi envias un enlace de invitación de grupo te pateare afuera y no bromeó 😎😉!', id)
                        }
                    } else if (args[0] == 'off') {
                        var cek = antilink.includes(chatId);
                        if(!cek){
                            return aruga.reply(from, '*ANTILINK* Se a desactivado', id) //if number already exists on database
                        } else {
                            let nixx = antilink.indexOf(chatId)
                            antilink.splice(nixx, 1)
                            fs.writeFileSync('./lib/helper/antilink.json', JSON.stringify(antilink))
                            aruga.reply(from, '*[Anti Group Link]* ha sido desactivado\n', id)
                        }
                    } else {
                    ...
                    break  
	case 'brainly':
            if (!isGroupMsg) return aruga.reply(from, 'Este comando solo se puede utilizar en grupos!', id)
        ...
        case 'stikergif':
            if (isMedia || isQuotedVideo) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    var mediaData = await decryptMedia(message, uaOverride)
                    aruga.reply(from, '[OJO] Awanta wee no soy Flash⏳ !', id)
                    ...
                    await exec(`gify ${filename} ./media/stickergf.gif --fps=30 --scale=240:240`, async function (error, stdout, stderr) {
                        ...
                        .catch(() => {
                            aruga.reply(from, 'ArchiLo siento, el archivo es demasiado grande!', id)
                        })
                    })
                  } else {
                    aruga.reply(from, `[❗] Enviar a pie de foto *${prefix}stickergif* max 10 sec!`, id)
                   }
                } else {
		    aruga.reply(from, `[❗] Enviar a pie de foto *${prefix}stickergif*`, id)
	        }
            break
        case 'stikergiphy':
        case 'stickergiphy':
            if (args.length !== 1) return aruga.reply(from, `Formato incorrecto.\nUsa${prefix}stickergiphy <link_giphy>`, id)
            const isGiphy = url.match(new RegExp(/https?:\/\/(www\.)?giphy.com/, 'gi'))
            const isMediaGiphy = url.match(new RegExp(/https?:\/\/media.giphy.com\/media/, 'gi'))
            if (isGiphy) {
                const getGiphyCode = url.match(new RegExp(/(\/|\-)(?:.(?!(\/|\-)))+$/, 'gi'))
                if (!getGiphyCode) { return aruga.reply(from, 'Error😖', id) }
                const giphyCode = getGiphyCode[0].replace(/[-\/]/gi, '')
                const smallGifUrl = 'https://media.giphy.com/media/' + giphyCode + '/giphy-downsized.gif'
                aruga.sendGiphyAsSticker(from, smallGifUrl).then(() => {
                    aruga.reply(from, 'Aca\'s Tu sticker')
                    console.log(`No debe tardar  ${processTime(t, moment())} Segundos`)
                }).catch((err) => console.log(err))
            } else if (isMediaGiphy) {
                const gifUrl = url.match(new RegExp(/(giphy|source).(gif|mp4)/, 'gi'))
                if (!gifUrl) { return aruga.reply(from, 'Error😖', id) }
                ...
                .catch(() => {
                    aruga.reply(from, `Algo se va a estrellar!`, id)
                })
            } else {
                await aruga.reply(from, 'Lo sentimos, la etiqueta de comando giphy sólo puede utilizar enlaces de giphy.  [Giphy Only]', id)
            }
            break
      case 'qrread':
        if (args.length !== 1) return aruga.reply(from, `Para utilizar el qrread, Use :\n${prefix}qrread url\n\nEjemplo: ${prefix}qrcode https://i.ibb.co/phSpp2h/00bed2bb-8b90-4d49-ace1-fe0ac9f73dff.jpg\n\n*Note : Upload terlebih dahaulu qrcode kamu ke https://id.imgbb.com/, kemudian copy url gambar qrcode kamu*`, id)
      ...
      break
    case 'qrcode':
        if (args.length !== 2) return aruga.reply(from, `Para utilizar el qrcode, ketik :\n${prefix}qrcode [kata/url] [size]\n\nEjemplo: ${prefix}qrcode https://google.com 300\n\n*Size minimal 100 & maksimal 500*`, id)
        aruga.reply(from, `wait...`, id);
      ...
      break			
        case 'meme':
            if ((isMedia || isQuotedImage) && args.length >= 2) {
                ...
                await aruga.reply(from, `Sin fotos! Por favor, envíe fotos con subtítulos ${prefix}meme <teks_atas> | <teks_bawah>\nEjemplo: ${prefix}meme Come algo | No tengo hambre`, id)
            }
            break
        case 'quotemaker':
	...
	//Group All User
	case 'grouplink':
            if (!isBotGroupAdmins) return aruga.reply(from, 'Bot necesita admin ', id)
            if (isGroupMsg) {
            	...
            	aruga.reply(from, 'Este comando solo se puede utilizar en grupos!', id)
            }
            break
	case "revoke":
	if (!isBotGroupAdmins) return aruga.reply(from, 'Bot necesita admin', id)
                    if (isBotGroupAdmins) {
                        aruga
                            ...
                            .then((res) => {
                                aruga.reply(from, `enlace cambiado con éxito*${prefix}grouplink* para obtener el último enlace de invitación de grupo`, id);
                            })
                            .catch((err) => {
                                console.log(`[ERR] ${err}`);
                            });
        ...
        //Media
        case 'ytmp3':
            if (args.length == 0) return aruga.reply(from, `Use para descargar canciones de YouTube\nEjemplo: ${prefix}ytmp3 [link_yt]`, id)
            const linkmp3 = args[0].replace('https://youtu.be/','').replace('https://www.youtube.com/watch?v=','')
			rugaapi.ytmp3(`https://youtu.be/${linkmp3}`)
            .then(async(res) => {
				if (res.error) return aruga.sendFileFromUrl(from, `${res.url}`, '', `${res.error}`)
				await aruga.sendFileFromUrl(from, `${res.result.thumb}`, '', `Canción encontrada🥳\n\nEncabezado: ${res.result.title}\nDescriocion: ${res.result.desc}\nPaciente lagi dikirim`, id)
				await aruga.sendFileFromUrl(from, `${res.result.url}`, '', '', id)
				.catch(() => {
					aruga.reply(from, `URL este ${args[0]} Ya se a descargado antes.Url se restablecerá después de 1 hora/ 60 minutos`, id)
				})
			})
            break
        case 'ytmp4':
            if (args.length == 0) return aruga.reply(from, `Use para descargar videos de YouTube\nEjemplo: ${prefix}ytmp3 [link_yt]`, id)
            const linkmp4 = args[0].replace('https://youtu.be/','').replace('https://www.youtube.com/watch?v=','')
			rugaapi.ytmp4(`https://youtu.be/${linkmp4}`)
            .then(async(res) => {
				if (res.error) return aruga.sendFileFromUrl(from, `${res.url}`, '', `${res.error}`)
				await aruga.sendFileFromUrl(from, `${res.result.thumb}`, '', `Encontrado🥳\n\nEncabezado: ${res.result.title}\nDescripcion: ${res.result.desc}\nPasiente lagi dikirim`, id)
				await aruga.sendFileFromUrl(from, `${res.result.url}`, '', '', id)
				.catch(() => {
					aruga.reply(from, `URL este ${args[0]} Ya se a descargado antes. Url se restablecerá después de 1 hora/ 60 minutos`, id)
				})
		...
		case 'fb':
		case 'facebook':
			if (args.length == 0) return aruga.reply(from, `Use para descargar videos de facebook\nEjemplo: ${prefix}fb [link_fb]`, id)
			rugaapi.fb(args[0])
			.then(async (res) => {
				const { link, linkhd, linksd } = res
				if (res.status == 'error') return aruga.sendFileFromUrl(from, link, '', 'Lo sentimos, su URL no se pudo encontrar', id)
				await aruga.sendFileFromUrl(from, linkhd, '', 'Aquí está el video', id)
				.catch(async () => {
					await aruga.sendFileFromUrl(from, linksd, '', 'Aquí está el video', id)
					.catch(() => {
						aruga.reply(from, 'Lo sentimos, su URL no se pudo encontrar', id)
					})
				})
			})
			break
	...
	case 'dewabatch':
		...
		//Fun Menu
    case 'tod':
    aruga.reply(from, 'Antes de cumplir la promesa de llevar a cabo cualquier orden.\n\nSilahkan Pilih:\n➥ #truth\n➥ #dare', id)
    break
    case 'truth':
    ...
    if (!isGroupMsg) return aruga.reply(from, menuId.textPrem())
        ...
        case 'kick':
            ...
            for (let i = 0; i < mentionedJidList.length; i++) {
                if (groupAdmins.includes(mentionedJidList[i])) return await aruga.sendText(from, 'Error, no puedes eliminar el administrador del grupo.')
        ...
        case 'tagall':
        case 'everyone':
            if (!isGroupMsg) return aruga.reply(from, 'Solo para el propietario!', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Solo para admins facheros 😎!', id)
            const groupMem = await aruga.getGroupMembers(groupId)
            let hehex = '╔══✪〘 Mesionar All 〙✪══\n'
		...
		case 'welcome':
			if (!isGroupMsg) return aruga.reply(from, 'solo para el propietario', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Solo para admins facheros 😎!', id)
            if (!isBotGroupAdmins) return aruga.reply(from, 'Bot requiere admin', id)
			...
			if (args[0] == 'on') {
				...
				aruga.reply(from, 'Mensaje de bienvenida ahora está habilitado!', id)
			} else if (args[0] == 'off') {
				let xporn = welcome.indexOf(chatId)
				welcome.splice(xporn, 1)
				fs.writeFileSync('./settings/welcome.json', JSON.stringify(welcome))
				aruga.reply(from, ''Mensaje de bienvenida ahora está desactivado', id)
			} else {
        ...
        if (!isGroupMsg) return aruga.reply(from, 'Este comando es sólo para el propietario del bot!', id)
        let isOwner = chat.groupMetadata.owner == pengirim
        if (!isOwner) return aruga.reply(from, 'Este comando solo puede ser usado por admins facheros 😎!', id)
        if (!isBotGroupAdmins) return aruga.reply(from, 'Bot requiere admin!', id)
            ...
            aruga.reply(from, 'JAJAJA miembro eliminado ', id)
        break

        //Owner Bot
        case 'ban':
            if (!isOwnerBot) return aruga.reply(from, 'Este comando es sólo para el propietario del bot!', id)
            if (args.length == 0) return aruga.reply(from, `Prohibir que alguien pudiera usar comandos\n\nCaranya ketik: \n${prefix}ban add 628xx --untuk mengaktifkan\n${prefix}ban del 628xx --untuk nonaktifkan\n\ncara cepat ban banyak digrup ketik:\n${prefix}ban @tag @tag @tag`, id)
            if (args[0] == 'add') {
                banned.push(args[1]+'@c.us')
                fs.writeFileSync('./settings/banned.json', JSON.stringify(banned))
                aruga.reply(from, 'EXITO!')
            } else
            if (args[0] == 'del') {
        ...
        case 'bc': //untuk broadcast atau promosi
            ...
            for (let idk of chatz) {
                ...
                if (!cvk.isReadOnly) aruga.sendText(idk, `〘 *SAMURAI-BOT* 〙\n\n${msg}`)
                if (cvk.isReadOnly) aruga.sendText(idk, `〘 *SAMURAI-BOT** 〙\n\n${msg}`)
            }
